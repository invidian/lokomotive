...
.PHONY: lint
lint:
	golangci-lint run --enable-all --disable=godox,lll --max-same-issues=0 --max-issues-per-linter=0 --build-tags aws,packet,e2e,disruptive-e2e --new-from-rev=$$(git merge-base master HEAD) --modules-download-mode=$(MOD) --timeout=5m ./...
